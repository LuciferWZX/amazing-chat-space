...
function useIMInit() {
  ...
  function listenMessage(message: Message) {
    ...
    requestAnimationFrame(() => {
      events.emit(`scroll-to-message`, message.messageID)
    })
  }
  function listen(packet: SendackPacket) {
    if (packet.reasonCode === 1) {
      console.warn('[消息发送成功]', packet)
    }
    else {
      console.error('[消息发送失败]', packet)
    }
    const messageId = packet.messageID
    const clientSeq = packet.clientSeq
    // 转数字
    const messageIdNumber = Number(messageId).toString()

    // 更新消息状态
    useIMStore.setState((oldState) => {
      const chatMessageMap = oldState.chatMessageMap
      // 遍历map，找到对应的message，更新状态
      const newMap = new Map()
      chatMessageMap.forEach((messages, channelId) => {
        ...
        newMap.set(channelId, _messages)
      })
