import { apis,types,instants,stores } from "@amazing-chat/shared";
...
const {ResponseCode,AppUser}=types
	...
	const { mutate, isPending } = useMutation({
		mutationFn: async (params:Parameters<typeof login>[0])=>{
			...
			if (response?.code === ResponseCode.SUCCESS ){
				...
				if (profileResponse?.code === ResponseCode.SUCCESS){
					//设置用户信息
					useAppStore.setState(()=>{
						return {...profileResponse.data,access_token:response.data.access_token} as AppUser
					})
					toast.success("登录成功", {
						icon:undefined,
						id: "login_id",
